...
export function calculateGradient() {
  ...
  const colorStops = sectionsArray
    .map((s, i) => {
      const start = getPositionInPercentage(s.start) - (i === 0 ? 0 : 0.5);
      const end =
        getPositionInPercentage(sectionsArray[i + 1]?.start ?? height) -
        (i === sectionsArray.length - 1 ? 0 : 1.5);
      return `${s.color} ${start}% , ${s.color} ${end}%`;
    })
    .join(", ");

  const gradient = `linear-gradient(to bottom, ${colorStops})`;
  return gradient;
}
